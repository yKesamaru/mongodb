...
これらの要素を組み合わせて、データベースの「構造」を形成します。それがスキーマです。

### コレクション
コレクションは、RDBのテーブルに相当するもので、ドキュメントが格納される場所です。一つのデータベース内に複数のコレクションを持つことができます。
はい、MongoDBにおいて各コレクションには名前をつける必要があります。コレクション名はそのコレクションが何を表しているのかを識別するためのものです。
